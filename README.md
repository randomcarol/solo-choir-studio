# Solo Choir Studio · 一个人的合唱团

手机端优先的可交互 Web 原型。用户可以选择《欢乐颂》《小星星》《友谊地久天长》三首公版完整旋律，完成声音测评、逐段练习、录制三个声部并在浏览器内同时播放。《彩虹》作为待授权曲目展示，不包含其旋律或录音。

这是一个 AI 音乐产品方向的工程原型，也是一次 AI 辅助开发实践。当前音高检测、对齐和合唱播放全部在浏览器本地完成，使用的是信号处理与确定性逻辑，不把它们误写成生成式 AI。
...
- [版本演进](DEVELOPMENT_HISTORY.md)：六个恢复快照及真实功能差异。
- [AI 协作与能力边界](docs/AI-DEVELOPMENT.md)：当前能力、AI 定位和下一阶段评测方式。
- [版权与选曲落地说明](docs/版权与选曲落地说明.md)：公版、授权和用户导入边界。
